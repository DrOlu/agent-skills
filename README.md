...
| `rmagent-redteam` | Purple-team DRILL for the rmagent-windows skill. Stages reversible living-off-the-land artifacts (failed admin logons, new local admin, SYSTEM scheduled task, LocalSystem service, PowerShell spawns, SYSTEM outbound connection) on a Windows estate, runs the defender to score detection, and Telegrams the report. Benign, RMAgentDrill_-prefixed, reversible. |
| `rmagent-windows` | Pull-based remote-witness habit ("RMAgent") for a Windows estate — ask five allowlisted named questions (attest, sketch, edges, explain, netedges) over pywinrm/WinRM :5985, tracking Administrator/SYSTEM. No log lake, no actuate; keeps the EDR. netedges reads the Sysmon EID3 ring to catch transient connections. |
