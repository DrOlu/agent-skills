...
Verified against cactus-needle 3.0.2. Import surface: `needle` exports
`Needle`, `tool`, `Field`, `extract`, `ExtractionValidationError`,
`__version__`.

## Table of contents

1. Defining tools
2. The Needle agent: `Needle(...)` and `run()`
3. Response shapes
...
`Needle` is a context manager (`with needle.Needle(tools=[...]) as agent:`)
and should be `close()`d when done. Only one Needle instance per generation
...
strict=True, generation=None)` builds a one-shot Needle around the schema and
...
Needle 3 only (`generation=2` raises). Use for local search/matching/routing
...
- `generation` — `2` (default legacy) vs `3`; plain `needle.Needle(...)`
  defaults to 3. `needle.Needle(tools=[...], generation=2)` runs Needle 2
