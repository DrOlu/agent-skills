#!/usr/bin/env python3
"""Generate a runnable needle instance from a neuralos-data profile + models.
...
def build_menu(profile, table, agent_name):
    ...
    menu = []
    I = lambda v, d=10: {"type": "integer", "description": v, "minimum": 1,
                         "maximum": 100 if d == 10 else d}
    S = lambda v, **kw: {"type": "string", "description": v, **kw}

    if kind == "database":
        # one table per instance: a 121M selector degrades beyond ~a dozen probes
        for t in [x for x in profile["source"].get("tables", []) if not table or x["name"] == table]:
            ...
            menu.append(menu_entry(
                ...
                {"limit": I("how many rows, e.g. 10", 100, ) if False else
                 {"type": "integer", "description": "how many rows, e.g. 10",
...
Generated by neuralos-data from profile.json at {now}.
"""
import csv
import io
import json
...
def {tn}_by_{c}(value: str) -> dict:
    column = "{col}"
    rows = _q("SELECT * FROM {q(tname)} WHERE `" + column + "` = '" + str(value) +
              "' LIMIT " + str(ROW_CAP))
    if rows and "error" in rows[0]:
        return rows[0]
...
def instance_code(profile, table, agent_name, example):
    ...
    return f'''"""neuralOS/needle instance for {agent_name} — generated by neuralos-data.
...
def selection():
    ...
    probes = [t for t in dir(bridge) if callable(getattr(bridge, t))
              and not t.startswith("_")]
    for phrasing in ["show me a summary", "give me the numbers", "summarize the data"]:
...
Data changed shape? Re-run the neuralos-data workflow (profile → model →
...
def main():
    ...
    verify_code = VERIFY.replace("{sample_call}", sample_call)
    if kind == "database":
