...
def _ask_ssh(row: dict, skill: str, since_hours: float, limit: int,
    ...
    try:
        ...
        return _cap({"ok": False, "error": f"ssh timeout after {timeout}s"}, row, skill) | {
    ...
    if r.returncode != 0:
        ...
        return _cap({"ok": False, "error": err or f"exit {r.returncode}"}, row, skill)
    ...
    for line in out.splitlines():
        if line.strip().startswith("{"):
            ...
            break
    if not jline:
        return _cap({"ok": False, "error": f"no JSON in output: {out[:200]}"}, row, skill)
    try:
        import json as _json
        parsed = _json.loads(jline)
    except Exception as e:
        return _cap({"ok": False, "error": f"json parse: {e}: {jline[:200]}"}, row, skill)
    return _cap({"ok": True, "data": parsed}, row, skill)
