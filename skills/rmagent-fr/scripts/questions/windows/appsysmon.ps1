...
$hashes=@(); $lsass=@(); $imgloads=@(); $regsets=@(); $guidconns=@(); $conns=@()
$n=0
try{
  if($svc){
    ...
    # REV 18 (M6): ALSO emit each connection in the canonical `conns` shape
    # ({dest,port,proc}) so correlate.py's lateral-hop join sees this view.
    # The ProcessGuid is the better join key (PIDs are reused); correlate
    # reads `conns`, so we provide both — Guid for identity, conns for join.
    ...
    Select -First $Limit|%{
      ...
      if($d['DestinationIp']){$conns+=[pscustomobject]@{dest=$d['DestinationIp'];port=$d['DestinationPort'];proc=$d['Image']}}
    }
...
[pscustomobject]@{skill='appsysmon';host=$hn;utc=$now;window_h=$sh;sysmon=$sysmon;n_events=$n;
  ...
  registry_sets=@($regsets);guid_conns=@($guidconns);conns=@($conns)}|ConvertTo-Json -Compress -Depth 4