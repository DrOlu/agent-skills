    ...
    try:
        with open(f, 'r', errors='replace') as fh:
            for i, line in enumerate(fh):
                ...
                if stan or rrn:
                    rows.append({"stan": stan, "rrn": rrn, "recv_ts": ts.group(1) if ts else None,
                                 "post_ts": None, "rc": rc, "pan": mask(panm.group(1) if panm else None)})
                if len(rows) >= limit: break
    except OSError:
        pass
    if len(rows) >= limit: break
print(json.dumps({"skill": "core_auth", "host": os.uname().nodename, "rows": rows[:limit]}, separators=(',', ':')))
PY
