...
# FIXES (2026-08-19): net.exe by FULL PATH (Impacket's net.py shadows `net` on ws1);
# failed_logons via LogonUser API (net use loopback generates NO 4625); per-signal
# verification so the scorer can't count silent failures as detections.
...
S 'failed_logons'    { Add-Type -MemberDefinition '[DllImport("advapi32.dll", SetLastError=true)] public static extern bool LogonUser(string u, string d, string p, int t, int pr, out IntPtr tok);' -Name RA -Namespace W; 1..2 | ForEach-Object { $t=[IntPtr]::Zero; [W.RA]::LogonUser('Administrator', $env:COMPUTERNAME, "Dr!ll-Bad-$_", 3, 0, [ref]$t) | Out-Null } }
