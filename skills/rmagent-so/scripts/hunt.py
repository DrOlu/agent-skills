...
def main():
    ...
    # --- rev 9.2: cross-witness correlation — join the answers we just pulled.
    # Pure post-processing over case answers (record_ask persists them), so it
    # works with the STC walker too: walked witnesses land in the same answers/.
    try:
        ...
        if adir.exists():
            for f in sorted(adir.glob("*.json")):
                try:
                    answers[f.stem] = json.loads(f.read_text())
                except Exception:
                    continue
        if any("__edges" in k for k in answers):
            c = corr.correlate(answers, rows)
            (case_dir / "correlation.json").write_text(json.dumps(c, indent=2))
            s = c.get("summary") or {}
            print(f"\n[correlate] critical={s.get('critical')} warning={s.get('warning')} total={s.get('total')}")
            for f in c.get("findings") or []:
                print("  [%-8s] %-22s %s" % (f.get("severity"), f.get("kind"), f.get("detail")))
    except Exception as e:
        print(f"[correlate] skipped ({e})")
