...
function RV($p,$n){$r=$null;try{$r=(Get-ItemProperty -Path $p -Name $n -ErrorAction SilentlyContinue).$n}catch{};return $r}
$script:o=@()
function A($t,$n,$v){
  $v=@($v|Where-Object{$_ -ne $null -and $_ -ne ''})
  if($v.Count){ $script:o+=[pscustomobject]@{t=$t;n=$n;c=$v.Count;v=@($v|Select-Object -First $Limit)} }
}
function RK($p){
  $r=@()
  try{
    $k=Get-ItemProperty -Path $p -ErrorAction SilentlyContinue
    if($k){ $r=@($k.PSObject.Properties|Where-Object{$_.Name -notmatch '^PS'}|ForEach-Object{"$($_.Name)=$($_.Value)"}) }
  }catch{}
  return $r
}
A 'T1547.001' 'run_keys' (@(RK "$H$R")+@(RK "$H$R`Once")+@(RK "$U$R")+@(RK "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run"))
$if=@()
try{foreach($c in (Get-ChildItem "$H$W\Image File Execution Options" -ErrorAction SilentlyContinue)){$d=(Get-ItemProperty -Path $c.PSPath -Name 'Debugger' -ErrorAction SilentlyContinue).Debugger;if($d){$if+="$($c.PSChildName)->$d"}}}catch{}
