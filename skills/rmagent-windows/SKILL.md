...
| **What connected?** (netedges) | `netedges.ps1` | SYSTEM/Administrator-owned outbound connections (**Sysmon EID3**) + DNS queries (**EID22**). Requires Sysmon with `<NetworkConnect onmatch="exclude">`. **Does not** return LSASS access, thread injection, file creates, or registry sets — that was a false claim; mutation of known persistence keys is `regedges` | the full netflow / packet capture |
| **What persistence already exists?** (attackmap) | `attackmap.ps1` | ATT&CK-mapped persistence **STATE** — ~16 registry locations (Run/RunOnce, IFEO, SilentProcessExit, cmd AutoRun, AppCert/AppInit, SSP, Winlogon, Netsh, port monitors, logon scripts). Every finding carries its technique ID | the whole hive |
| **Who turned a latch?** (regedges) | `regedges.ps1` | path-allowlisted **Sysmon 13** (Registry value set) on those same latches, tracked principals only, plus `sysmon_reg` (`ok`/`blind`/`absent`). Watch-only — never writes | a hive dump or unbounded EID13 |
...
> `edges` reads *currently-Established* connections — a point-in-time snapshot that misses sub-second connections. `netedges` reads the **Sysmon ring** (EID3 + EID22 only). Use `netedges` for transient outbound; use `regedges` for who SetValue'd a known persistence latch. Both stay pull-only, both capped — no lake. Registry **writes** are actuate/redteam, never so.
