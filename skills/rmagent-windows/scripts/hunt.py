...
def main():
    ...
    # Rev 8: cross-witness correlation — join the answers we just pulled so
    # lateral-movement patterns (same account on both boxes, WS1->WS2 edges,
    # shared LogonId) surface. Pure post-processing of this case's answers.
    try:
        import correlate as corr
        answers = corr._load_answers(case_dir)
        if any("__edges" in k for k in answers):
            ...
            s = c["summary"]
            print(f"\n[correlate] critical={s['critical']} warning={s['warning']} total={s['total']}")
            for f_ in c["findings"][:10]:
                print(f"  [{f_['severity']:8}] {f_['kind']:22} {f_['detail']}")
