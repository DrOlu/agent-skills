...
def walk(inv: dict, start_id: str, since_h: float, limit: int) -> dict:
    ...
    for _ in range(MAX_HOPS):
        ...
        # REV 20 (P0): a MISSING blind_count is UNKNOWN, not sighted. The old
        # `int(ad.get("blind_count") or 0)` treated an attest that could not
        # answer the blind question as a clean bill and walked straight through
        # a possibly-blind witness to claim "origin".
        att = lib.ask(row, "attest", since_hours=1.0, limit=5)
        ad = att.get("data") or {}
        if not att.get("ok"):
            hops.append({
                "host": current, "blind": True,
                "note": f"attest failed ({att.get('error') or 'no answer'}) — sightedness unknown; refusing to claim origin here",
            })
            termination = "attest-unavailable"
            break
        raw_blind = ad.get("blind_count")
        if raw_blind is None or not str(raw_blind).strip():
            hops.append({
                "host": current, "blind": True,
                "note": "attest returned no blind_count — sightedness UNKNOWN (not sighted); refusing to claim origin here",
            })
            termination = "blind-unknown"
            break
        blind = int(raw_blind)
        if blind > 0:
            hops.append({
    ...
    return {
        ...
        "confidence": ("high" if termination == "origin" and not retention_limited
                       else "low" if termination in ("retention-boundary", "blind-witness",
                                                     "no-signal", "blind-unknown",
                                                     "attest-unavailable")
                       else "medium"),
        "note": {
            ...
            "blind-unknown": ("a witness on the path could not report its sightedness — treated as "
                              "unknown, not sighted; verify audit policy, re-walk"),
            "attest-unavailable": ("a witness on the path could not be attested at all — no origin "
                                   "claim can pass through it"),
